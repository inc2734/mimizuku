...
add_filter( 'inc2734_wp_seo_google_tag_manager_id', function( $tag_manager_id ) {
	...
	if ( $for_loggedin ) {
		$output = apply_filters( 'mimizuku_output_google_tag_manager', ! current_user_can( 'manage_options' ) );
		if ( ! $output ) {
			return;
		}
	}
...
add_filter( 'inc2734_wp_seo_google_analytics_tracking_id', function( $tracking_id ) {
	...
	if ( $for_loggedin ) {
		$output = apply_filters( 'mimizuku_output_google_analytics', ! current_user_can( 'manage_options' ) );
		if ( ! $output ) {
			return;
		}
	}
